...
export default {
    ...
    created(){
        ...
        console.log(this.$moment().endOf('month').format("YYYY-MM-DD")+" 23:59:59",this.$moment().startOf('month').format('YYYY-MM-DD HH:mm:ss'));
        this.handleApiCis();
    ...
    methods:{
        ...
        lastMonth(month,year){
            ...
            return parseInt(month)===12?`${year++}-01-20`:`${year}-${parseInt(month)-1}-20`;
        },
        handleApiCis(){
        ...
        handleApiLastCis(){
            console.log('sadfdasfa',this.$moment(this.today).format('DD'),this.lastMonth(this.$moment().format('MM'),this.$moment().format('YYYY')));
            let day = this.$moment(this.today).format('DD'),now,last;
                
            if(day>20){       
                // this.$moment().endOf('month').format("YYYY-MM-DD")+" 23:59:59"
                now =   this.$moment().endOf('month').format("YYYY-MM-DD")+" 23:59:59";
                now = this.$moment(now).format('YYYY-MM-DD HH:mm:ss');
                last =this.$moment().startOf('month').format('YYYY-MM-DD HH:mm:ss');               
            }else{
                let temp_month = this.$moment(`${this.lastMonth(this.$moment().format('MM'),this.$moment().format('YYYY'))} ${this.timeline}`).format('MM'),
                temp_year = this.$moment(`${this.lastMonth(this.$moment().format('MM'),this.$moment().format('YYYY'))} ${this.timeline}`).format('YYYY');
                last =  this.$moment(`${this.lastMonth(temp_month,temp_year)} ${this.timeline}`).format();
                now = this.lastMonth(this.$moment().format('MM'),this.$moment().format('YYYY'))+' 00:00:00';
                now = this.$moment(now).format('YYYY-MM-DD HH:mm:ss');
            }
